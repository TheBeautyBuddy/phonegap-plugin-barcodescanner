...
- (UIView*)buildOverlayView {
    ...
    UILabel* myLabel = [[UILabel alloc]initWithFrame:CGRectMake(30, 50, 400, 40)];
...
- (UIImage*)buildReticleImage {
    ...
    if (self.processor.is2D) {
        UIColor* color = [UIColor colorWithRed:0.0 green:0.4 blue:0.6 alpha:RETICLE_ALPHA];
        CGContextSetStrokeColorWithColor(context, color.CGColor);
        CGContextSetLineWidth(context, RETICLE_WIDTH);
        CGRect roundRect =  CGRectMake(
                                       RETICLE_OFFSET,
                                       RETICLE_OFFSET,
                                       RETICLE_SIZE-2*RETICLE_OFFSET,
                                       RETICLE_SIZE-2*RETICLE_OFFSET
                                       );
        addRoundedRectToPath(context, roundRect, 5, 5);
